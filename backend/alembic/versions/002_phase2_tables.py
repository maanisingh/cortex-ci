...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE effectseverity AS ENUM ('negligible', 'minor', 'moderate', 'significant', 'severe', 'catastrophic')"
    )
    op.execute(
        "CREATE TYPE aianalysistype AS ENUM ('anomaly', 'pattern', 'summary', 'scenario', 'clustering')"
    )
    op.execute(
        "CREATE TYPE aianalysisstatus AS ENUM ('pending', 'processing', 'completed', 'failed', 'awaiting_approval', 'approved', 'rejected')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "scenario_chain_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("scenario_chains.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "entity_id",
        ...
        sa.Column(
            "caused_by_effect_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("chain_effects.id", ondelete="SET NULL"),
        ),
    ...
    op.create_index(
        "ix_risk_justifications_entity", "risk_justifications", ["entity_id"]
    )
    op.create_index(
        "ix_risk_justifications_tenant", "risk_justifications", ["tenant_id"]
    )
    ...
    op.create_index(
        "ix_historical_snapshots_entity", "historical_snapshots", ["entity_id"]
    )
    op.create_index(
        "ix_historical_snapshots_date", "historical_snapshots", ["snapshot_date"]
    )
    ...
    op.create_index(
        "ix_constraint_changes_constraint", "constraint_changes", ["constraint_id"]
    )
    ...
    op.create_index(
        "ix_anomaly_detections_reviewed", "anomaly_detections", ["is_reviewed"]
    )
