...
def upgrade() -> None:
    # Create enum types
    op.execute(
        "CREATE TYPE userrole AS ENUM ('admin', 'analyst', 'approver', 'viewer')"
    )
    op.execute(
        "CREATE TYPE entitytype AS ENUM ('organization', 'individual', 'location', 'asset', 'system')"
    )
    op.execute(
        "CREATE TYPE constrainttype AS ENUM ('policy', 'regulation', 'compliance', 'contractual', 'operational', 'financial', 'security', 'custom')"
    )
    op.execute(
        "CREATE TYPE constraintseverity AS ENUM ('low', 'medium', 'high', 'critical')"
    )
    op.execute(
        "CREATE TYPE dependencylayer AS ENUM ('legal', 'financial', 'operational', 'academic', 'human', 'technical')"
    )
    op.execute("CREATE TYPE risklevel AS ENUM ('low', 'medium', 'high', 'critical')")
    op.execute(
        "CREATE TYPE scenariostatus AS ENUM ('draft', 'running', 'completed', 'archived')"
    )
    op.execute(
        "CREATE TYPE scenariotype AS ENUM ('entity_restriction', 'country_embargo', 'supplier_loss', 'dependency_failure', 'custom')"
    )
    op.execute(
        "CREATE TYPE auditaction AS ENUM ('create', 'read', 'update', 'delete', 'login', 'logout', 'export', 'calculate')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "constraint_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("constraints.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_entity_constraints_constraint", "entity_constraints", ["constraint_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "calculated_at", sa.DateTime(timezone=True), server_default=sa.text("now()")
        ),
    )
    ...
    op.create_index(
        "ix_risk_history_entity_date", "risk_history", ["entity_id", "recorded_at"]
    )
