...
from sqlalchemy import String, Text, Integer, Float, ForeignKey, Date, Boolean
...
class Framework(Base, TimestampMixin, TenantMixin):
    ...
    # OSCAL support
    oscal_catalog_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    oscal_data: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)

    # Relationships
    controls = relationship("Control", back_populates="framework", cascade="all, delete-orphan")
    assessments = relationship("Assessment", back_populates="framework", cascade="all, delete-orphan")
...
class Control(Base, TimestampMixin, TenantMixin):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    framework_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("frameworks.id", ondelete="CASCADE"), index=True)
    ...
    # Hierarchy
    parent_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("controls.id"), nullable=True)
    family: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    category: Mapped[Optional[ControlCategory]] = mapped_column(String(50), nullable=True)

    # Control details
    guidance: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    supplemental_guidance: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    # Classification
    baseline_impact: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # LOW, MODERATE, HIGH
    priority: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # P1, P2, P3

    # Implementation
    implementation_status: Mapped[AssessmentResult] = mapped_column(String(50), default=AssessmentResult.NOT_ASSESSED)
    ...
    # Related artifacts
    test_procedures: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String), nullable=True)
    references: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String), nullable=True)

    # MITRE ATT&CK specific
    tactics: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String), nullable=True)
    techniques: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String), nullable=True)
    mitigations: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String), nullable=True)

    # OSCAL data
    oscal_control_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    oscal_data: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)

    # Relationships
    framework = relationship("Framework", back_populates="controls")
    parent = relationship("Control", remote_side=[id], backref="children")
    evidence_links = relationship("EvidenceLink", back_populates="control", cascade="all, delete-orphan")

    # Mappings
    source_mappings = relationship("ControlMapping", foreign_keys="ControlMapping.source_control_id", back_populates="source_control")
    target_mappings = relationship("ControlMapping", foreign_keys="ControlMapping.target_control_id", back_populates="target_control")
...
class ControlMapping(Base, TimestampMixin, TenantMixin):
    ...
    source_control_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("controls.id", ondelete="CASCADE"), index=True)
    target_control_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("controls.id", ondelete="CASCADE"), index=True)

    relationship_type: Mapped[MappingRelationship] = mapped_column(String(50), nullable=False)
    confidence: Mapped[float] = mapped_column(Float, default=1.0)

    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    source: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # UCF, NIST, manual
...
class Assessment(Base, TimestampMixin, TenantMixin):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    assessment_type: Mapped[str] = mapped_column(String(50), nullable=False)  # self, external, certification
    ...
    # Status
    status: Mapped[AssessmentStatus] = mapped_column(String(50), default=AssessmentStatus.NOT_STARTED)
    ...
    assessor_organization: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)

    # Scope
    scope_description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    in_scope_controls: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String), nullable=True)
