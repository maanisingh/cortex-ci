...
class Vendor(Base, TimestampMixin, TenantMixin):
    ...
    status: Mapped[VendorStatus] = mapped_column(
        String(50), default=VendorStatus.PROSPECTIVE, index=True
    )

    # Category
    category: Mapped[str] = mapped_column(
        String(100), nullable=False
    )  # Technology, Professional Services, etc.
    ...
    country: Mapped[str | None] = mapped_column(String(3), nullable=True, index=True)
    ...
    # Financial
    annual_spend: Mapped[Decimal | None] = mapped_column(Numeric(20, 2), nullable=True)
    ...
    # Internal ownership
    business_owner_id: Mapped[UUID | None] = mapped_column(
    ...
    questionnaire_responses = relationship(
        "QuestionnaireResponse", back_populates="vendor", cascade="all, delete-orphan"
    )
...
class VendorAssessment(Base, TimestampMixin, TenantMixin):
    ...
    # Risk determination
    risk_rating_before: Mapped[str | None] = mapped_column(String(20), nullable=True)
    risk_rating_after: Mapped[str | None] = mapped_column(String(20), nullable=True)
    ...
    findings_detail: Mapped[dict | None] = mapped_column(JSONB, nullable=True)

    # Recommendations
    recommendations: Mapped[str | None] = mapped_column(Text, nullable=True)
    required_actions: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
...
class QuestionnaireResponse(Base, TimestampMixin, TenantMixin):
    ...
    questionnaire_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("vendor_questionnaires.id"), index=True
    )
...
class VendorContract(Base, TimestampMixin, TenantMixin):
    ...
    contract_type: Mapped[str] = mapped_column(
        String(100), nullable=False
    )  # MSA, SOW, SLA, NDA, DPA, BAA

    # Status
    status: Mapped[ContractStatus] = mapped_column(
        String(50), default=ContractStatus.DRAFT, index=True
    )
    ...
    data_protection_terms: Mapped[str | None] = mapped_column(Text, nullable=True)
    liability_cap: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    # Internal owner
    contract_owner_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
