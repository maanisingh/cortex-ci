...
class Incident(Base, TimestampMixin, TenantMixin):
    ...
    status: Mapped[IncidentStatus] = mapped_column(
        String(50), default=IncidentStatus.NEW, index=True
    )
    ...
    )  # SIEM, User Report, Monitoring, etc.
    detection_source: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    data_types_affected: Mapped[list[str]] = mapped_column(
    ...
    reputation_impact: Mapped[str | None] = mapped_column(String(50), nullable=True)

    # Technical details
    attack_vector: Mapped[str | None] = mapped_column(String(255), nullable=True)
    indicators_of_compromise: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    ...
    breach_determination_notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    breach_determined_by: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
    breach_determined_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )

    # External reporting
    reported_to_law_enforcement: Mapped[bool] = mapped_column(Boolean, default=False)
    law_enforcement_case_number: Mapped[str | None] = mapped_column(String(100), nullable=True)
    reported_to_regulators: Mapped[bool] = mapped_column(Boolean, default=False)
    regulator_reference: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class IncidentTimeline(Base, TimestampMixin, TenantMixin):
    ...
    )  # EVENT, ACTION, COMMUNICATION, STATUS_CHANGE
    title: Mapped[str] = mapped_column(String(500), nullable=False)
...
class IncidentResponse(Base, TimestampMixin, TenantMixin):
    ...
    # Playbook reference
    playbook_step: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class BreachNotification(Base, TimestampMixin, TenantMixin):
    ...
    notification_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # INDIVIDUAL, REGULATOR, MEDIA, LAW_ENFORCEMENT

    # Recipient
    recipient_type: Mapped[str] = mapped_column(
        String(100), nullable=False
    )  # DPA, State AG, HHS, Customer, etc.
    ...
    notification_deadline: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )

    # Status
    status: Mapped[NotificationStatus] = mapped_column(
        String(50), default=NotificationStatus.PENDING_ASSESSMENT
    )
    ...
    delivery_confirmation: Mapped[str | None] = mapped_column(String(255), nullable=True)
