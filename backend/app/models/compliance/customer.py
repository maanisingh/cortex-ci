...
class Customer(Base, TimestampMixin, TenantMixin):
    ...
    status: Mapped[CustomerStatus] = mapped_column(
        String(50), default=CustomerStatus.PROSPECT, index=True
    )
    risk_rating: Mapped[CustomerRiskRating] = mapped_column(
        String(50), default=CustomerRiskRating.MEDIUM, index=True
    )
    ...
    country_of_residence: Mapped[str | None] = mapped_column(String(3), nullable=True, index=True)

    # Organization fields
    legal_name: Mapped[str | None] = mapped_column(String(500), nullable=True, index=True)
    ...
    incorporation_country: Mapped[str | None] = mapped_column(String(3), nullable=True, index=True)
    ...
    pep_type: Mapped[str | None] = mapped_column(String(100), nullable=True)
    ...
    sanction_details: Mapped[str | None] = mapped_column(Text, nullable=True)

    has_adverse_media: Mapped[bool] = mapped_column(Boolean, default=False)
    adverse_media_details: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    expected_activity: Mapped[str | None] = mapped_column(Text, nullable=True)
    expected_volume: Mapped[str | None] = mapped_column(String(255), nullable=True)

    # Beneficial ownership (for organizations)
    beneficial_owners: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    ...
    onboarded_by: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
    offboarded_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    offboarded_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    screening_results = relationship(
        "ScreeningResult", back_populates="customer", cascade="all, delete-orphan"
    )
...
class CustomerReview(Base, TimestampMixin, TenantMixin):
    ...
    # Before/after risk
    previous_risk_rating: Mapped[CustomerRiskRating | None] = mapped_column(
        String(50), nullable=True
    )
    new_risk_rating: Mapped[CustomerRiskRating | None] = mapped_column(String(50), nullable=True)
    ...
    # Checklist
    checklist_completed: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
