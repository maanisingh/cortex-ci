...
class Audit(Base, TimestampMixin, TenantMixin):
    ...
    # Auditor info
    lead_auditor_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    ...
    # Internal contacts
    audit_sponsor_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    internal_lead_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    ...
    # Reports
    draft_report_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    final_report_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
...
class AuditFinding(Base, TimestampMixin, TenantMixin):
    ...
    status: Mapped[FindingStatus] = mapped_column(String(50), default=FindingStatus.OPEN, index=True)
    ...
    control_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("controls.id"), nullable=True)

    # Risk
    inherent_risk: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    ...
    target_remediation_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    actual_remediation_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    ...
    response_by: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    ...
    risk_accepted_by: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    risk_accepted_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)

    # Repeat finding
    is_repeat: Mapped[bool] = mapped_column(Boolean, default=False)
    previous_finding_id: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("audit_findings.id"), nullable=True)
...
class RemediationPlan(Base, TimestampMixin, TenantMixin):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    finding_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("audit_findings.id", ondelete="CASCADE"), index=True)

    # Plan details
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[RemediationStatus] = mapped_column(String(50), default=RemediationStatus.DRAFT, index=True)

    # Actions
    action_steps: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
    ...
    last_updated_by: Mapped[Optional[UUID]] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
