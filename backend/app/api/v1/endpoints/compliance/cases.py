...
from app.models.compliance.case import (
    Case, CasePriority, CaseStatus, CaseType,
    CaseNote, CaseTask, TaskStatus, SARReport, SARStatus
)
...
async def list_cases(
    ...
    result = await db.execute(
        query.order_by(Case.created_at.desc()).offset(skip).limit(limit)
    )
...
async def list_case_notes(
    ...
    result = await db.execute(
        select(CaseNote).where(
            and_(CaseNote.case_id == case_id, CaseNote.tenant_id == tenant_id)
...
async def list_case_tasks(
    ...
    result = await db.execute(
        select(CaseTask).where(
            and_(CaseTask.case_id == case_id, CaseTask.tenant_id == tenant_id)
...
async def complete_case_task(
    ...
    result = await db.execute(
        select(CaseTask).where(
            and_(CaseTask.id == task_id, CaseTask.case_id == case_id, CaseTask.tenant_id == tenant_id)
        )
    )
...
async def list_sars(
    ...
    result = await db.execute(
        query.order_by(SARReport.created_at.desc()).offset(skip).limit(limit)
    )
    return result.scalars().all()
