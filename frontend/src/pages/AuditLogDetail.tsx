...
export default function AuditLogDetail() {
  ...
  const { data: log, isLoading, error } = useQuery<AuditLog>({
  ...
  if (error || !log) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        <button onClick={() => navigate('/audit')} className="mt-2 text-red-600 underline">
          Back to Audit Log
        </button>
      </div>
    )
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="card mb-6">
        ...
        <dl className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <dd className="font-medium capitalize">{log.resource_type?.toLowerCase()}</dd>
          </div>
          <div>
            <dt className="text-sm text-gray-500">Resource</dt>
            <dd className="font-medium">{log.resource_name || log.resource_id || 'N/A'}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="font-medium">{new Date(log.created_at).toLocaleString()}</dd>
          </div>
          <div>
            <dt className="text-sm text-gray-500">IP Address</dt>
            <dd className="font-medium font-mono text-sm">{log.ip_address || 'N/A'}</dd>
          </div>
        </dl>
      </div>
      ...
      {log.user_agent && (
        <div className="card">
          ...
          <p className="text-sm text-gray-600 font-mono break-all">{log.user_agent}</p>
        </div>
      )}
    </div>
  )
