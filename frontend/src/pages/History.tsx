...
export default function History() {
  const [activeTab, setActiveTab] = useState<
    "decisions" | "timeline" | "constraints"
  >("decisions");
  ...
  const { data: decisions, isLoading: decisionsLoading } = useQuery({
    queryKey: ["decisions"],
    queryFn: async () => {
      const response = await historyApi.decisions.list({
  ...
  const createDecisionMutation = useMutation({
    mutationFn: (data: typeof decisionForm) =>
      historyApi.decisions.create(data),
    onSuccess: () => {
      ...
      <div className="sm:flex sm:items-center mb-6">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700">
            Decision tracking, timeline views, and organizational knowledge
            preservation
          </p>
        </div>
        {activeTab === "decisions" && (
                      ...
                      <div className="bg-gray-50 p-3 rounded-lg">
                        ...
                        {event.risk_score_before !== undefined && (
                          <div className="text-xs mt-2">
                            Risk: {event.risk_score_before?.toFixed(1)} -&gt;{" "}
                            {event.risk_score_after?.toFixed(1)}
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {activeTab === "constraints" && (
        <div className="card">
          <h3 className="font-semibold mb-4">
            Recent Constraint Changes (Last 90 Days)
          </h3>
          {constraintsLoading ? (
            <div className="text-center py-8 text-gray-500">
              Loading changes...
            ...
            <div className="text-center py-8 text-gray-500">
              No constraint changes in the last 90 days
            </div>
          ) : (
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                createDecisionMutation.mutate(decisionForm);
              }}
            >
              <div className="space-y-4">
                <div>
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg">
            ...
            <div className="mb-4 p-3 bg-gray-50 rounded-lg">
              ...
              <div className="font-medium">
                {selectedDecision.decision_summary}
              </div>
            </div>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                recordOutcomeMutation.mutate(outcomeForm);
              }}
            >
              <div className="space-y-4">
                <div>
