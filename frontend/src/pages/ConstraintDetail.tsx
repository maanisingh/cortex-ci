    ...
    try {
      ...
      try {
        ...
        const affected = allEntities.filter((entity: Entity) => {
          const typeMatch = constraintRes.data.applies_to_entity_types?.length === 0 ||
            constraintRes.data.applies_to_entity_types?.includes(entity.type)
          const countryMatch = constraintRes.data.applies_to_countries?.length === 0 ||
            constraintRes.data.applies_to_countries?.includes(entity.country_code)
          const categoryMatch = constraintRes.data.applies_to_categories?.length === 0 ||
  ...
  if (error || !constraint) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        <button onClick={() => navigate('/constraints')} className="mt-2 text-red-600 underline">
          Back to Constraints
        </button>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">{constraint.name}</h1>
          <div className="flex items-center gap-2 mt-1">
            <span className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${getTypeColor(constraint.type)}`}>
              {constraint.type}
            </span>
            {constraint.reference_code && (
              <span className="text-gray-500 text-sm">Ref: {constraint.reference_code}</span>
            )}
          </div>
        </div>
        <div className="flex items-center space-x-3">
          <div className={`px-4 py-2 rounded-lg border ${getSeverityColor(constraint.severity)}`}>
            <div className="text-xs uppercase font-medium">Severity</div>
            <div className="text-lg font-bold capitalize">{constraint.severity}</div>
          </div>
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${constraint.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
      ...
      {/* Expiry Warning */}
      {constraint.expiry_date && isExpiringSoon(constraint.expiry_date) && !isExpired(constraint.expiry_date) && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <p className="text-orange-700 font-medium">
            This constraint expires on {new Date(constraint.expiry_date).toLocaleDateString()}
          </p>
        </div>
      )}
      {constraint.expiry_date && isExpired(constraint.expiry_date) && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-700 font-medium">
            This constraint expired on {new Date(constraint.expiry_date).toLocaleDateString()}
          </p>
        </div>
      )}
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow p-6">
          ...
          <dl className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <dd className="font-medium">{constraint.is_mandatory ? 'Yes' : 'No'}</dd>
            </div>
            <div>
              <dt className="text-sm text-gray-500">Effective Date</dt>
              <dd className="font-medium">
                {constraint.effective_date ? new Date(constraint.effective_date).toLocaleDateString() : 'N/A'}
              </dd>
            </div>
            <div>
              <dt className="text-sm text-gray-500">Expiry Date</dt>
              <dd className="font-medium">
                {constraint.expiry_date ? new Date(constraint.expiry_date).toLocaleDateString() : 'No expiry'}
              </dd>
            </div>
            <div>
              <dt className="text-sm text-gray-500">Review Date</dt>
              <dd className="font-medium">
                {constraint.review_date ? new Date(constraint.review_date).toLocaleDateString() : 'Not set'}
              </dd>
            </div>
            <div>
              <dt className="text-sm text-gray-500">Created</dt>
              <dd className="font-medium">{new Date(constraint.created_at).toLocaleDateString()}</dd>
            </div>
            ...
            {constraint.external_url && (
              <div className="col-span-2">
                ...
                <dd>
                  <a href={constraint.external_url} target="_blank" rel="noopener noreferrer"
              ...
              {constraint.applies_to_entity_types?.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {constraint.applies_to_entity_types.map(type => (
                    <span key={type} className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-sm capitalize">
                      {type.toLowerCase()}
                    </span>
                  ))}
                </div>
              ...
              {constraint.applies_to_countries?.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {constraint.applies_to_countries.map(country => (
                    <span key={country} className="px-2 py-0.5 bg-green-100 text-green-700 rounded text-sm">
                      {country}
                    </span>
                  ))}
                </div>
              ...
              {constraint.applies_to_categories?.length > 0 ? (
                <div className="flex flex-wrap gap-1">
                  {constraint.applies_to_categories.map(cat => (
                    <span key={cat} className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-sm">
                      {cat}
                    </span>
                  ))}
                </div>
      ...
      {constraint.requirements && Object.keys(constraint.requirements).length > 0 && (
        <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {affectedEntities.length > 0 ? (
            ...
            {affectedEntities.map(entity => (
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    <div className="font-medium text-primary-600">{entity.name}</div>
                    <div className="text-sm text-gray-500">
                      {entity.type} • {entity.country_code || 'N/A'} • {entity.category || 'Uncategorized'}
                    </div>
                  </div>
                </div>
              </Link>
          ...
          <p className="text-gray-500 text-center py-4">No entities match the constraint criteria</p>
        )}
      </div>
