      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 capitalize">
            {entity.type.toLowerCase()} • {entity.category || "Uncategorized"}
          </p>
        </div>
        <div className="flex items-center space-x-3">
          {riskScore && (
            <div
              className={`px-4 py-2 rounded-lg border ${getRiskColor(riskScore.risk_level)}`}
            >
          ...
          <span
            className={`px-3 py-1 rounded-full text-sm font-medium ${entity.is_active ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
          >
          ...
          <dl className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <dd className="font-medium">
                {getCriticalityLabel(entity.criticality)} ({entity.criticality}
                /5)
              </dd>
            </div>
            <div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {riskScore.constraint_score?.toFixed(1) || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Dependency Risk</span>
                  <span className="font-medium">
                    {riskScore.dependency_score?.toFixed(1) || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Country Risk</span>
                  <span className="font-medium">
                    {riskScore.country_score?.toFixed(1) || "N/A"}
                  </span>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-medium text-primary-600">
                        {dep.target_entity_name || "Unknown Entity"}
                      </div>
                    ...
                    <div className="text-right">
                      <span className="text-xs bg-gray-100 px-2 py-1 rounded capitalize">
                        {dep.layer.toLowerCase()}
                      </span>
                      <div className="text-xs text-gray-500 mt-1">
                        Criticality: {dep.criticality}/5
                      </div>
                    </div>
                  </div>
                </Link>
              ...
              {incomingDeps.map((dep) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-medium text-primary-600">
                        {dep.source_entity_name || "Unknown Entity"}
                      </div>
                      <div className="text-sm text-gray-500 capitalize">
                        {dep.relationship_type.toLowerCase().replace(/_/g, " ")}
                      </div>
