          ...
          {!selectedEntity ? (
            <div className="card text-center py-12">
              ...
              <p className="mt-4 text-gray-500">
                Select an entity to view risk justification
              </p>
            </div>
            ...
            <div className="card text-center py-12">
              ...
              <p className="mt-4 text-gray-500">
                No justification available for this entity
              </p>
            </div>
          ) : justification ? (
            <div className="space-y-4">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <h2 className="text-xl font-bold">
                      {justification.entity_name}
                    </h2>
                    <div className="flex items-center gap-4 mt-2">
                      <div className="text-3xl font-bold">
                        {justification.risk_score.toFixed(1)}
                      </div>
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${riskLevelColors[justification.level]}`}
                      >
                        {justification.level}
                      </span>
                  ...
                  {justification.justification.analyst_can_override && (
                    <button
                      onClick={() => {
                        setOverrideData({
                          new_score: justification.risk_score,
                          reason: "",
                        });
                ...
                {justification.has_override && justification.override_info && (
                  <div className="mt-4 p-3 bg-purple-50 rounded-lg">
                    <div className="text-sm">
                      ...
                      {justification.override_info.original_score.toFixed(1)}
                      <span className="mx-2">-&gt;</span>
                      <span className="font-medium">Override:</span>{" "}
                      {justification.override_info.override_score.toFixed(1)}
                    </div>
                    <div className="text-sm text-gray-600 mt-1">
                      {justification.override_info.override_reason}
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      By {justification.override_info.overridden_by} on{" "}
                      {new Date(
                        justification.override_info.overridden_at,
                      ).toLocaleString()}
                    </div>
                  </div>
                )}
              ...
              <div className="card">
                ...
                <div className="space-y-3">
                  {justification.justification.primary_factors.map(
                    (factor: RiskFactor, i: number) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="card">
                  ...
                  <div
                    className={`text-3xl font-bold ${confidenceColor(justification.justification.uncertainty.confidence)}`}
                  >
                    {(
                      justification.justification.uncertainty.confidence * 100
                    ).toFixed(0)}
                    %
                  </div>
                  <div className="mt-3 space-y-1">
                    ...
                    {justification.justification.uncertainty.factors.map(
                      (f: string, i: number) => (
                        <div
                          key={i}
                ...
                <div className="card">
                  ...
                  <ul className="space-y-2">
                    {justification.justification.assumptions.map(
              ...
              <div className="text-xs text-gray-500 text-right">
                Generated:{" "}
                {new Date(
                  justification.justification.generated_at,
                ).toLocaleString()}
              </div>
            </div>
          ) : null}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setOverrideData({
                        ...overrideData,
                        new_score: parseFloat(e.target.value),
                      })
                    }
                    required
                  />
