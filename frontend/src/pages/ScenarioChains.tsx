...
export default function ScenarioChains() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string; description?: string; trigger_event: string }) =>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-4xl my-8 mx-4">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => setSimulationResult(null)} className="text-gray-400 hover:text-gray-600">
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">{simulationResult.total_entities_affected}</div>
                <div className="text-sm text-gray-600">Entities Affected</div>
              </div>
              <div className="bg-orange-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-orange-600">{simulationResult.max_cascade_depth}</div>
                <div className="text-sm text-gray-600">Cascade Depth</div>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">{simulationResult.estimated_timeline_days}</div>
                <div className="text-sm text-gray-600">Days Timeline</div>
              </div>
              <div className="bg-red-50 p-4 rounded-lg">
                <span className={`inline-flex px-2 py-1 rounded text-sm font-medium ${severityColors[simulationResult.overall_severity]}`}>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {simulationResult.immediate_effects.map((effect: any, i: number) => (
                    <div key={i} className="bg-red-50 p-3 rounded border border-red-100">
                      <div className="font-medium">{effect.entity_name}</div>
                      <div className="text-sm text-gray-600">{effect.effect_description}</div>
                      <div className="text-xs mt-1">
                        <span className={`px-1.5 py-0.5 rounded ${severityColors[effect.severity]}`}>
                          {effect.severity}
                        </span>
                        <span className="ml-2">Risk +{effect.risk_score_delta}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {simulationResult.delayed_effects.map((effect: any, i: number) => (
                    <div key={i} className="bg-orange-50 p-3 rounded border border-orange-100">
                      <div className="font-medium">{effect.entity_name}</div>
                      <div className="text-sm text-gray-600">{effect.effect_description}</div>
                      <div className="text-xs mt-1">
                        <span className={`px-1.5 py-0.5 rounded ${severityColors[effect.severity]}`}>
                        ...
                        <span className="ml-2">+{effect.time_delay_days} days</span>
                        <span className="ml-2">Depth: {effect.cascade_depth}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-6 pt-4 border-t">
              ...
              <div className="text-sm text-gray-600">
                Total Risk Increase: <span className="font-bold text-red-600">+{simulationResult.risk_impact_summary.total_risk_increase}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-4 text-center text-gray-500">
                    No scenario chains found. Create one to start cascade analysis.
                  </td>
                </tr>
              ) : (
                data.map((chain: ScenarioChain) => (
                  <tr key={chain.id} className="hover:bg-gray-50">
                    <td className="table-cell">
                      ...
                      {chain.description && (
                        <div className="text-xs text-gray-500 truncate max-w-xs">{chain.description}</div>
                      )}
                    </td>
                    <td className="table-cell">
                      <div className="text-sm text-gray-600 truncate max-w-xs">{chain.trigger_event}</div>
                    </td>
                    ...
                    <td className="table-cell">
                      <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${severityColors[chain.overall_severity] || 'bg-gray-100'}`}>
