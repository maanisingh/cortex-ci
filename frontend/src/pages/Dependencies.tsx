...
const layerColors: Record<string, { bg: string; border: string; edge: string }> = {
...
export default function Dependencies() {
  ...
  const { data: graphData, isLoading } = useQuery<GraphData>({
    ...
    queryFn: async () => {
      const response = await dependenciesApi.graph({ entity_id: entityId || undefined, depth: 3 })
  ...
  const { nodes: flowNodes, edges: flowEdges } = useMemo(() => {
    ...
    const nodes: Node[] = graphData.nodes.map((node, index) => {
      ...
      const riskColor = node.risk_level ? riskColors[node.risk_level.toLowerCase()] : null
        ...
        data: {
          label: (
            <div className="text-center">
              <div className="font-medium text-sm truncate max-w-[120px]" title={node.label}>
                {node.label}
              </div>
              <div className="text-xs text-gray-500 capitalize">{node.type?.toLowerCase()}</div>
              {node.risk_level && (
                <div
                  className="text-xs mt-1 px-1 rounded"
                  style={{ backgroundColor: riskColor || '#e5e7eb', color: '#fff' }}
                >
                  {node.risk_level}
                </div>
              )}
    ...
    const edges: Edge[] = graphData.edges.map((edge) => {
      const layerStyle = layerColors[edge.layer?.toLowerCase()] || layerColors.operational
      return {
        id: edge.id,
        source: edge.source,
        target: edge.target,
        ...
        style: { stroke: layerStyle.edge, strokeWidth: Math.max(1, edge.criticality / 2) },
        markerEnd: {
    ...
    <div>
      <div className="sm:flex sm:items-center mb-6">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700">
            ...
            {entityId && (
              <span className="ml-2">
                • <Link to="/dependencies" className="text-primary-600 hover:underline">Clear filter</Link>
              </span>
            )}
          </p>
        </div>
            ...
            {Object.entries(graphData?.stats?.layers || {}).map(([layer, count]) => (
              <span
                ...
                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${
                  layerColorClasses[layer.toLowerCase()] || 'bg-gray-100 text-gray-800'
                }`}
              >
      ...
      <div className="card mb-4">
        <div className="flex flex-wrap gap-6 text-sm">
          ...
          <div>
            <span className="font-medium text-gray-700">Dependency Layers:</span>
            <div className="flex gap-3 mt-1">
              {Object.entries(layerColors).map(([layer, colors]) => (
                <span key={layer} className="flex items-center gap-1">
                  <span className="w-4 h-0.5" style={{ backgroundColor: colors.edge }} />
      ...
      <div className="mt-4 text-sm text-gray-500">
        <p><strong>Tips:</strong> Click and drag to pan. Scroll to zoom. Click on a node to view entity details. High-criticality dependencies are animated.</p>
      </div>
    </div>
  )
}
