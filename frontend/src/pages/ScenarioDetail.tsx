  ...
  const loadScenarioData = async () => {
    try {
      ...
      if (scenarioRes.data.trigger_entity_id) {
        try {
          const entityRes = await entitiesApi.get(scenarioRes.data.trigger_entity_id)
  ...
  if (error || !scenario) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        <button onClick={() => navigate('/scenarios')} className="mt-2 text-red-600 underline">
          Back to Scenarios
        </button>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">{typeLabels[scenario.type] || scenario.type}</p>
        </div>
        <div className="flex items-center space-x-3">
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(scenario.status)}`}>
            {scenario.status}
          </span>
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow p-6">
          ...
          <dl className="grid grid-cols-2 gap-4">
            <div>
              ...
              <dd className="font-medium">{typeLabels[scenario.type] || scenario.type}</dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd className="font-medium">{new Date(scenario.created_at).toLocaleString()}</dd>
            </div>
            {scenario.completed_at && (
              <div>
                <dt className="text-sm text-gray-500">Completed</dt>
                <dd className="font-medium">{new Date(scenario.completed_at).toLocaleString()}</dd>
              </div>
            )}
            {triggerEntity && (
              <div className="col-span-2">
                ...
                <dd>
                  <Link to={`/entities/${triggerEntity.id}`} className="text-primary-600 hover:underline font-medium">
                    {triggerEntity.name}
                  </Link>
                  <span className="text-gray-500 ml-2">({triggerEntity.type})</span>
                </dd>
              </div>
            )}
              ...
              <div className="col-span-2">
                ...
                <dd className="flex flex-wrap gap-1 mt-1">
                  {scenario.affected_entity_types.map(type => (
                    <span key={type} className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-sm capitalize">
                      {type.toLowerCase()}
                    </span>
                  ))}
                </dd>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{scenario.results.cascade_depth} levels</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Execution Time</span>
                  <span className="font-medium">{scenario.results.execution_time_ms}ms</span>
                </div>
              </div>
              {scenario.results.risk_delta && Object.keys(scenario.results.risk_delta).length > 0 && (
                <div className="border-t pt-3">
                  <h3 className="text-sm text-gray-500 mb-2">Risk Changes</h3>
                  {Object.entries(scenario.results.risk_delta).map(([key, value]) => (
      ...
      {/* Affected Entities */}
      {scenario.results?.affected_entities && scenario.results.affected_entities.length > 0 && (
        <div className="bg-white rounded-lg shadow p-6">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cascade Level</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Risk Change</th>
                ...
                {scenario.results.affected_entities.map((entity, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <Link to={`/entities/${entity.entity_id}`} className="text-primary-600 hover:underline font-medium">
                        {entity.entity_name}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${getImpactColor(entity.impact_level)}`}>
                        {entity.impact_level}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm">
                      <span className={entity.risk_after > entity.risk_before ? 'text-red-600' : 'text-green-600'}>
                        {entity.risk_before?.toFixed(1)} → {entity.risk_after?.toFixed(1)}
                      </span>
                    </td>
      ...
      {scenario.status === 'archived' && (scenario.outcome_notes || scenario.lessons_learned) && (
