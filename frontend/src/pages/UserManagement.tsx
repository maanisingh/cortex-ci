...
export default function UserManagement() {
  ...
  return (
    <div>
      ...
      <Modal
        isOpen={showForm}
        onClose={() => { setShowForm(false); setEditUser(null); resetForm(); }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Admin: Full access | Analyst: Create/Edit | Approver: Approve changes | Viewer: Read-only
            </p>
          </div>

          <div className="flex justify-end space-x-3 pt-4 border-t">
            <button type="button" onClick={() => { setShowForm(false); setEditUser(null); resetForm(); }} className="btn-secondary">
              Cancel
            </button>
            <button type="submit" disabled={createMutation.isPending || updateMutation.isPending} className="btn-primary">
              {(createMutation.isPending || updateMutation.isPending) ? 'Saving...' : editUser ? 'Update User' : 'Create User'}
            </button>
          </div>
        </form>
      </Modal>
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-600">
            Are you sure you want to delete <strong>{showDeleteConfirm?.full_name}</strong>? This action cannot be undone.
          </p>
