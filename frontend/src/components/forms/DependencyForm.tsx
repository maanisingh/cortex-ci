...
const relationshipTypes = [
  'SUPPLIES_TO', 'SOURCES_FROM', 'SUBSIDIARY_OF', 'PARTNER_OF', 'OWNS',
  'CONTROLS', 'PROVIDES_SERVICE_TO', 'RECEIVES_SERVICE_FROM', 'TRANSACTS_WITH',
  'EMPLOYS', 'CONTRACTS_WITH', 'INVESTS_IN', 'RECEIVES_INVESTMENT_FROM'
]
...
export default function DependencyForm({ isOpen, onClose, dependency, preselectedSourceId, preselectedTargetId }: DependencyFormProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => ({
      ...
      [name]: type === 'checkbox' ? (e.target as HTMLInputElement).checked :
              name === 'criticality' ? parseInt(value) : value,
    }))
  }

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Edit Dependency' : 'Add New Dependency'} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t">
          ...
          <button type="submit" disabled={mutation.isPending} className="btn-primary">
            {mutation.isPending ? 'Saving...' : isEdit ? 'Update Dependency' : 'Create Dependency'}
          </button>
        </div>
      </form>
    </Modal>
